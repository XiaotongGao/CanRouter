...
const uint8_t m_data_length_map[16] = {0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 16, 20, 24, 32, 48, 64};
...
#if (FOR_WM_HDMAP == ON)
void special_map_for_G10(CANRxMsg_t* RxMsg)
{
	if (RxMsg->id == 0x418)
	{
		// reset id
		...
		// reset data length
		RxMsg->dlc = CAN_LEN8_DLC;
		// store the leagcy value to temperary variable
		uint8_t leagcy_value = RxMsg->data8[0];
		uint8_t new_value = 0x0;
		// reset out put data buffer
		memset(RxMsg->data8, 0x00, m_data_length_map[CAN_LEN8_DLC]);

		if (leagcy_value == 0x50)
		{
			new_value = 0x0;
		}
		else if (leagcy_value == 0x52)
		{
			new_value = 0x2;
		}
		else if (leagcy_value == 0x4E)
		{
			new_value = 0x3;
		}
		else if ( (leagcy_value == 0x44) 
			|| (leagcy_value == 0x35) 
			|| (leagcy_value == 0x34) 
			|| (leagcy_value == 0x33) 
			|| (leagcy_value == 0x32) 
		...
		RxMsg->data8[3] = new_value;
	}
	else if ((RxMsg->id == 0x200) || (RxMsg->id == 0x208))
	{
		// reset data length
		RxMsg->dlc = CAN_LEN8_DLC;
		uint16_t RTKspeed_H = 0;
		uint16_t RTKspeed_L = 0;
		// reset id
		if (RxMsg->id == 0x200)
		{
			RxMsg->id = 0x348;
			// unpack the message to get the signal
			RTKspeed_H = (uint16_t)((( ((RxMsg->data8[2] & 0x3F) * 256) + RxMsg->data8[3] ) * 0.5 *3.14 * 0.7074 * 60 / 1000) / 0.03125);
			RTKspeed_L = (uint16_t)((( ((RxMsg->data8[4] & 0x3F) * 256) + RxMsg->data8[5] ) * 0.5 *3.14 * 0.7074 * 60 / 1000) / 0.03125);
		}
		else
		{
			RxMsg->id = 0x34A;
			// unpack the message to get the signal
			RTKspeed_H = (uint16_t)((( ((RxMsg->data8[4] & 0x3F) * 256) + RxMsg->data8[5] ) * 0.5 *3.14 * 0.7074 * 60 / 1000) / 0.03125);
			RTKspeed_L = (uint16_t)((( ((RxMsg->data8[6] & 0x3F) * 256) + RxMsg->data8[7] ) * 0.5 *3.14 * 0.7074 * 60 / 1000) / 0.03125);
		}
		...
		memset(RxMsg->data8, 0x00, m_data_length_map[CAN_LEN8_DLC]);	
		// set frame data
		RxMsg->data8[0] = RTKspeed_H >> 8;
		RxMsg->data8[1] = RTKspeed_H;	
		RxMsg->data8[2] = RTKspeed_L >> 8;
		RxMsg->data8[3] = RTKspeed_L;
